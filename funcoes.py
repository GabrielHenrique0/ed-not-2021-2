...
# Existem funções já providas pela lingugagem, como, por exemplo,
...
def imc(peso, altura): # Definição (ou declaração) da função
    ...
    # comentário chamado docstring, e servem para documentar o propósito de uma função ou classe.
...
def area_forma(base, altura, forma = 'R'):
    """
        Função que calcula a área de uma das seguintes formas geométricas: retângulo, triângulo ou elipse
        Parâmetro forma:
        "R" == retãngulo (parâmetro opcional com valor padrão)
        "T" == triângulo
        "E" == elipse
    """
